...
def disparity_filter(G, weight = 'weight'):
    ''' 
    Compute significance scores (alpha) for weighted edges in a NetworkX graph as defined in Serrano et al. 2009.

    Args:
        G (NetworkX Graph or DiGraph): A weighted graph, either undirected or directed.
        weight (str, optional): The attribute name specifying edge weights. Default is 'weight'.

    Returns:
        NetworkX Graph or DiGraph: A weighted graph with a significance score (alpha) assigned to each edge.
...
def disparity_filter_alpha_cut(G, weight = 'weight', alpha_t = 0.4, cut_mode = 'or'):
    ...
    Returns
    -------
        B (NetworkX Graph or DiGraph): The resulting graph contains only edges that survived from the filtering with the alpha_t threshold
